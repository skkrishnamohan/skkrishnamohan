# 👋 Hi, I'm Krishnamohan S

**Senior Azure DevOps Engineer** | **Kubernetes & Cloud Infrastructure Expert**  
🚀 14 Years in Linux, Automation & Multi-Cloud | Azure | Terraform | CI/CD | DevSecOps

---

## 💼 About Me

I'm a seasoned **DevOps Engineer** with 14 years of experience in building scalable, secure, and automated infrastructure using **Azure**, **Kubernetes**, and **Linux**. My mission is to help businesses innovate faster with modern DevOps strategies and cloud-native architectures.

---

## 🚀 What I Do Best

- 🔁 **CI/CD Pipelines** – Azure DevOps, GitHub Actions, YAML pipelines  
- ☁️ **Cloud Infrastructure** – Azure | AWS | Multi-Cloud | Hybrid Deployments  
- 🐳 **Containers & Orchestration** – Kubernetes | Docker | Helm | AKS  
- 📈 **Monitoring & Logging** – Prometheus | Grafana | OpenSearch  
- 🔐 **DevSecOps** – SonarCloud | BlackDuck | Trivy | Checkov | Azure Firewall  
- 📜 **Infrastructure as Code** – Terraform | Ansible
-    **Progamming Language** - Python
- 🐧 **Linux Expertise** – RHEL, Ubuntu, CentOS | Shell Scripting | System Hardening  

---

## 🛠️ Tech Stack

`Azure DevOps` `Kubernetes` `Terraform` `Docker`  
`Helm` `PowerShell` `Azure CLI` `Git`  
`Prometheus` `Grafana` `Linux` `PostgreSQL` `MySQL`

---

## 📜 Certifications

- 🎓 Cybersecurity Architect Expert
- 🎓 Azure Solutions Architect Expert  
- 🎓 DevOps Engineer Expert
- 🎓 Security Operations Analyst Associate
- 🎓 Azure Developer Associate
- 🎓 Azure Administration Associate
- 🎓 Azure Fundamentals

---

## 🌍 Open To

💼 Remote   

---

## 📫 Let's Connect

- 🔗 [LinkedIn](https://www.linkedin.com/in/skkrishnamohan)  
- 📧 Email: skkrishnamohan@gmail.com  

---

⭐ *Thanks for visiting my GitHub! Feel free to explore my repos or drop a message if you'd like to collaborate on a DevOps or Cloud project.*  
